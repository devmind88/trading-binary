...
export const ProfessionalAnalytics: React.FC<ProfessionalAnalyticsProps> = ({ 
  ...
  const [activeView, setActiveView] = useState<'quant_metrics' | 'predictive_forecaster'>('quant_metrics');
  ...
  return (
    <div id="professional-analytics-command-center" className="space-y-6">
      ...
      <div className="flex flex-wrap items-center justify-between gap-3 bg-slate-900 border border-slate-800 p-2.5 rounded-xl shadow-lg">
        <div className="flex items-center gap-2">
          <button
            onClick={() => setActiveView('quant_metrics')}
            className={`flex items-center gap-2 px-4 py-2 rounded-lg text-xs font-mono font-bold transition cursor-pointer ${
              activeView === 'quant_metrics'
                ? 'bg-emerald-950 text-emerald-300 border border-emerald-800 shadow'
                : 'text-slate-400 hover:text-slate-200 hover:bg-slate-850'
            }`}
          >
            <BarChart3 className="w-4 h-4 text-emerald-400" />
            <span>QUANT RISK DECOMPOSITION</span>
          </button>

          <button
            onClick={() => setActiveView('predictive_forecaster')}
            className={`flex items-center gap-2 px-4 py-2 rounded-lg text-xs font-mono font-bold transition cursor-pointer ${
              activeView === 'predictive_forecaster'
                ? 'bg-purple-950 text-purple-300 border border-purple-800 shadow'
                : 'text-slate-400 hover:text-slate-200 hover:bg-slate-850'
            }`}
          >
            <Sparkles className="w-4 h-4 text-purple-400 animate-pulse" />
            <span>PREDICTIVE SETUP FORECASTER</span>
            <span className="text-[9px] font-mono px-1.5 py-0.2 rounded bg-purple-900/60 text-purple-200 font-normal">
              ALPHA AI
            </span>
          </button>
        </div>
        ...
        {activeView === 'quant_metrics' && (
          <div className="flex items-center gap-3">
            ...
            <button
              ...
              className="px-3 py-1.5 rounded-lg bg-slate-950 hover:bg-slate-850 border border-slate-700 hover:border-emerald-500 text-emerald-300 transition text-xs font-mono flex items-center gap-1.5 cursor-pointer shadow"
            >
      ...
      {activeView === 'predictive_forecaster' ? (
        <PredictiveAnalytics trades={trades} />
      ) : (
        <>
          ...
          <div className="bg-slate-900 border border-slate-800 rounded-xl p-5 shadow-2xl flex flex-col md:flex-row md:items-center justify-between gap-4">
            ...
            <button
              onClick={() => setActiveView('predictive_forecaster')}
              className="px-3.5 py-2 rounded-lg bg-purple-950/80 hover:bg-purple-900 border border-purple-800 text-purple-200 transition text-xs font-mono flex items-center gap-2 shrink-0 cursor-pointer"
            >
              <Sparkles className="w-3.5 h-3.5 text-purple-400" />
              <span>Forecast Next Trade Edge &rarr;</span>
            </button>
          </div>
